...
object PR{ // PR (Prefs) 本地磁盘轻量键值SP数据持久化薄操作封装单例
	...
	var hs:String // hs (HistoryStorage) 历史足迹 JSON 序列化长字符串
		set(v){sp.edit().putString("his_json",v).apply()} // 落地保存
...
fun sH(){ // sH (saveHistory) 本地持久化序列化历史记录池方法
	...
	PR.hs=j.toString() // 强行压入SP磁盘
}
fun lH(){ // lH (loadHistory) 进场本地反序列化方法
	FN.hi.clear()
	runCatching{ // 防护 JSON 损坏
...
fun aH(im:FN.VT){ // 历史记录内存更新追加方法
	FN.hi.removeAll{it.id==im.id} // 先行干掉旧有同主键项
	FN.hi.add(0,im) // 推入第0位
	sH() // 增量同步磁盘
}
fun rH(id:String){FN.hi.removeAll{it.id==id};sH()} // 定向擦除单条并存盘
fun cH(){FN.hi.clear();sH()} // 归零当前全量足迹并存盘
