...
@Composable private fun X(){ // NV (NavHostComponent) 应用声明式全局路由骨干网拓扑分发控制中心组件
	...
	androidx.compose.foundation.layout.Box(modifier="fs".css()){ // 全景绝对满幅包裹的UI最外层箱盒子根节点
		...
		androidx.compose.foundation.layout.Box(modifier="fw pnb".css(this),contentAlignment=androidx.compose.ui.Alignment.BottomCenter){FN.LP()} // 贴底且规避底部导航区域的日志悬浮面板定位包裹盒
	}
}
